...
#[derive(Clone, Debug)]
pub struct AquesTalkDllRaw(Arc<libloading::Library>);
...
impl AquesTalkDllRaw {
    pub fn new<P: AsRef<OsStr>>(filename: P) -> Result<AquesTalkDllRaw, libloading::Error> {
        ...
        Ok(AquesTalkDllRaw(Arc::new(lib)))
    }

    pub unsafe fn synthe(&self, koe: *const c_char, speed: c_int) -> (*const c_uchar, usize) {
        let lib = &self.0;
    ...
    pub unsafe fn free_wave(&self, wav: *const c_uchar) {
        let lib = &self.0;
